...
public class PropertiesUtil {
     ...
     * 右下角水印字体(宋体).
     */
    public static String waterFont ;
    /**
     * 点选文字验证码的文字字体(宋体).
     */
    ...
    public void init() {
        ...
        try {
            URLPROPS.load(PropertiesUtil.class.getResourceAsStream("/application.properties"));
            applicationName = "/application.properties";
        } catch (IOException e) {
            e.printStackTrace();
        }

        InputStream is = PropertiesUtil.class.getResourceAsStream(applicationName);
        try {
            URLPROPS.load(is);
            jigsaw = getString("captcha.jigsaw","classpath:images/jigsaw");
