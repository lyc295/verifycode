(function($, window, document,undefined) {
	...
	function checkPictrue(data,baseUrl){
		return new Promise((resolve,reject)=>{
			$.ajax({
				type : "post",
				contentType: "application/json;charset=UTF-8",
				url : baseUrl + "/captcha/checkByImg",
    ...
    Slide.prototype = {
        ...
        refresh: function() {
			...
			this.isEnd = false;
			getPictrue(this.options.baseUrl).then(res=>{
				if (res.code==10000) {
					this.$element.find(".backImg")[0].src = 'data:image/png;base64,'+res.responseBody.originalImageBase64
					this.$element.find(".bock-backImg")[0].src = 'data:image/png;base64,'+res.responseBody.jigsawImageBase64
					this.secretKey = res.responseBody.secretKey
					this.backToken = res.responseBody.token
				}
			});
			this.htmlDoms.sub_block.css('left', "0px");
        },
    ...
    Points.prototype = {
        ...
        refresh: function() {
			...
			getPictrue({captchaType:"clickWord"},_this.options.baseUrl).then(res=>{
				if (res.code==10000) {
					this.htmlDoms.back_img[0].src ='data:image/png;base64,'+ res.responseBody.originalImageBase64
					this.backToken = res.responseBody.token
					this.secretKey = res.responseBody.secretKey
					let text = '请依次点击【' + res.responseBody.wordList.join(",") + '】'
